...
$env:DEEPSEEK_AI_ENABLED="1"
$env:DEEPSEEK_TIMEOUT_SECONDS="20"
```

请只在本机环境变量或 Streamlit secrets 中配置真实 API key，不要写入仓库文件。Streamlit Cloud 中建议设置 `DEEPSEEK_API_KEY`、`DEEPSEEK_BASE_URL`、`DEEPSEEK_MODEL`、`DEEPSEEK_AI_ENABLED`、`DEEPSEEK_TIMEOUT_SECONDS`；代码也兼容常见别名 `DEEPSEEK_APIKEY` 和 `[deepseek] api_key`。启用后，治理合规资质二级页的关键治理亮点、公司详情页的“主要风险与减分项”会优先使用 DeepSeek 生成结果；如果未设置 API key，或 API 超时/异常，页面会自动回退到规则/数据库文案，不影响页面展示。
